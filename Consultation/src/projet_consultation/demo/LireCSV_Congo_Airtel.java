...
public class LireCSV_Congo_Airtel
{
    public void lireFichierCSV() throws SQLException
    {
        try
        {
            ConnexionBDDOperateur cn=new ConnexionBDDOperateur("airtelbdd");
             String chemin = "C:\\Users\\SFM\\Desktop\\code_java\\Congo\\CSV_Congo\\CONGOBRAZA_AIRTEL.csv";
            ...
            while((chaine = fichier_source.readLine())!= null)
            {
                if(i > 1)
                {
                    ...
                    if (chaine!=null)
                    {
                        ...
                        localite = tabChaine[2].trim().replaceAll("'","\n");
                        nom_site = tabChaine[2].trim().replaceAll("'","\n");
                    }
                    String val1=verifierValeur(tabChaine[9]);
                    String val=val1.replace(",",".");
                    longitude=Double.parseDouble(val);

                    val1=verifierValeur(tabChaine[8]);
                    val=val1.replace(",",".");
                    ...
                    String requete="insert into table_bts(nom,localite,longitude,latitude,the_geom) values('"+nom_site+"','"+localite+"',"+longitude+","+latitude+",GeometryFromText('POINT("+longitude+" "+latitude+")',-1))";
        ...
        {
            Logger.getLogger(LireCSV_Congo_Airtel.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void EssaiMultiLineString() throws SQLException
    {
        ...
        for (int i = 0; i < tab.length; i++)
        {
            ...
            {
                Logger.getLogger(LireCSV_Congo_Airtel.class.getName()).log(Level.SEVERE, null, sQLException);
            }
        }
    }
    ...
    public void SetMultiLineString() throws SQLException
    {
        ConnexionBDDOperateur cn=new ConnexionBDDOperateur("tunisianabdd");
    ...
    {
        try {
            LireCSV_Congo_Airtel lr = new LireCSV_Congo_Airtel();
            lr.lireFichierCSV();
            //lr.SetRegion();
            //lr.SetRegion();
